...
// POST /documents/ingest  { title, source_type, text, uploaded_by }
// The frontend extracts text from the uploaded PDF/DOCX (client-side) and posts it here.
// We chunk, embed (via the AI service), and store rows in document_chunks.
documentsRouter.post("/ingest", async (req, res) => {
  const { title, source_type = "ucg", text, uploaded_by } = req.body ?? {};
  ...
  const { data: doc, error } = await supabase
    .from("documents")
    .insert({ title, source_type, uploaded_by: uploaded_by ?? null, status: "processing" })
    .select()
    .single();
  if (error || !doc) {
...
// ---- Retrieval: find the most relevant UCG chunks for a question ----
export async function retrieveContext(question: string, matchCount = 6): Promise<ContextChunk[]> {
  // 1. Embed the question
  const { embedding } = await ai.embed(question);
  if (!embedding.length) return [];

  // 2. Vector similarity search via the SQL function
